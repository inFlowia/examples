...
define('MSG_NO_CONTENT', "<div class = 'bad_news'>Такого на инФловии нет.
	<p>И откуда вы только эти ссылки битые берёте...</p></div>"); // конкретное сообщение об ошибке - оно выводится в первую очередь
define('MSG_NO_CONTENT_GENERAL', "<div class = 'bad_news'>Что-то пошло не так ':(</div>"); // общее сообщение об ошибке пользователю оно выводится в последнюю очередь.


/* Важно! При переименовании каталога, нужно поправить .htaccess, строку RewriteRule ^(.*)$ /имя_каталога/index.php [L,QSA] */
try{ // получение идент. контента из УРЛ и запросы к БД
	...
	switch (count($urlParts)){
		case 1: // если перешли просто в директорию - редир. на гл. (потом можно что-то придумать поумнее)
			echo '<meta http-equiv="refresh" content="0;URL=http://'.$_SERVER['HTTP_HOST'].'/">';
			die;
		case 2: // если ид. контента передан
